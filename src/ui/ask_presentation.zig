...
pub const Runtime = struct {
    ...
    pub fn init(
        ...
        const layout = zeroFooterLayout(try ui_terminal.queryLayout(
            if (builtin.os.tag == .windows) std.Io.File.stdout().handle else std.posix.STDOUT_FILENO,
            0,
        ));
    ...
    fn refreshGeometry(self: *Runtime) !void {
        const queried = ui_terminal.queryLayout(
            if (builtin.os.tag == .windows) std.Io.File.stdout().handle else std.posix.STDOUT_FILENO,
            0,
        ) catch return;
